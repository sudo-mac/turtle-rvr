...
class InfraredControlObserver:
    ...
    def start_infrared_broadcasting(self, far_code, near_code):
        ...
        self.__rvr.start_robot_to_robot_infrared_broadcasting(
            far_code.value, near_code.value
        )
    ...
    def start_infrared_following(self, far_code, near_code):
        ...
        self.__rvr.start_robot_to_robot_infrared_following(
            far_code.value, near_code.value
        )
    ...
    def send_infrared_messages(self, messages, strength=0):
        ...
        if strength < 0 or strength > 64:
            raise ValueError(
                "parameter strength must be greater than or equal to 0 and less than or equal 64"
            )

        for message in messages:
            self.__rvr.send_infrared_message(
                message.value, strength, strength, strength, strength
            )
